  ...
  useEffect(() => {
    api.get("/predios/lugares-produccion")
      .then(res => {
        setLugares(res.data)
        if (preSelectedId) {
          setFormData(prev => ({ ...prev, id_lugar_produccion: preSelectedId }))
        }
      })
              ...
              <CardContent className="p-10 space-y-10">
                ...
                <div className="space-y-4">
                  <div className="flex items-center gap-3">
                     ...
                     <label className="text-xs font-black text-slate-400 uppercase tracking-widest italic">Seleccionar Predio Autorizado</label>
                  </div>
                  <div className="relative">
                    ...
                    >
                      <option value="">¿Qué predio requiere inspección?</option>
                      {lugares.length > 0 ? lugares.map(l => (
                        <option key={l.id_lugar_produccion} value={l.id_lugar_produccion}>{l.nombre_lugar}</option>
                      )) : (
                        <option disabled>No se encontraron predios registrados</option>
                      )}
                    </select>
                  </div>
                </div>
