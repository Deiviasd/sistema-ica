    ...
    <div className="space-y-8 pb-10">
      <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }}>
        <div className="flex flex-col md:flex-row md:items-end justify-between gap-6 mb-2">
          <div className="flex items-center gap-4">
            ...
            <div>
              ...
              <p className="text-muted-foreground font-medium uppercase tracking-widest text-[10px]">
                {viewStatus === 'inactivo' 
                  ? 'Verificación y Aprobación Administrativa • ICA Hub'
          ...
          <div className="flex bg-muted p-1 rounded-2xl border border-border self-start md:self-auto gap-1">
             <button 
                onClick={() => setViewStatus('inactivo')}
                className={`flex items-center gap-2 px-4 py-2 rounded-xl text-xs font-bold transition-all ${viewStatus === 'inactivo' ? 'bg-primary text-primary-foreground italic' : 'text-muted-foreground hover:text-foreground'}`}
             >
      ...
      {users.length === 0 ? (
        <Card className="bg-card backdrop-blur-xl border-border border-dashed rounded-[2rem] shadow-sm">
        ...
        <div className="grid gap-6 md:grid-cols-2 lg:grid-cols-3">
          {users.map((user, index) => (
            ...
            >
              <Card className={`bg-card backdrop-blur-2xl border-border transition-all duration-500 rounded-[2rem] overflow-hidden group-hover:translate-y-[-4px] shadow-sm hover:shadow-md ${viewStatus === 'inactivo' ? 'hover:border-primary/50' : 'hover:border-rose-500/50'}`}>
                <div className="p-6">
                  <div className="flex justify-between items-start mb-6">
                    <div className={`p-3 rounded-2xl border transition-colors ${viewStatus === 'inactivo' ? 'bg-muted border-border group-hover:border-primary/30' : 'bg-rose-500/5 border-rose-500/20 group-hover:border-rose-500/40'}`}>
                      {viewStatus === 'inactivo' 
                  ...
                  <div className="space-y-1 mb-6">
                    <h3 className="text-xl font-bold tracking-tight leading-tight">{user.nombre}</h3>
            ...
            >
                           ...
                           {selectedUser.usuario_predio && selectedUser.usuario_predio.length > 0 ? (
                              <div className="flex items-center justify-between">
                                 ...
                                 <div className="text-right">
                                    <span className="text-[10px] bg-emerald-500 text-emerald-950 font-black px-3 py-1 rounded-full italic">VALIDADO</span>
                                 </div>
                              </div>
                           ) : (
               ...
               <div className="p-8 bg-muted/50 border-t border-border flex gap-4">
                  <Button
                     ...
                     className="flex-1 h-16 rounded-2xl border-2 border-border bg-transparent text-muted-foreground hover:bg-rose-500/10 hover:text-rose-500 hover:border-rose-500/30 transition-all font-black uppercase tracking-widest text-xs"
                  >
