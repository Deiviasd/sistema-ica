      ...
      <div className="grid gap-8 lg:grid-cols-3">
        ...
        <div className="space-y-6">
          ...
          <div className="space-y-4 max-h-[600px] overflow-y-auto pr-2 custom-scrollbar">
            {predios.flatMap(p => p.lote || []).filter(l => {
              const siembraActiva = siembras.find(s => s.id_lote === l.id_lote && !s.fecha_fin);
              return !!siembraActiva; // Solo mostramos los que tienen siembra activa
            }).map((l) => {
              const siembraActiva = siembras.find(s => s.id_lote === l.id_lote && !s.fecha_fin);
              const predio = predios.find(p => p.lote?.some(lot => lot.id_lote === l.id_lote));

              return (
                <Card key={l.id_lote} className="bg-slate-900/30 border-slate-800/50 transition-all">
                  <CardContent className="p-4 flex items-center gap-4">
                    <div className="w-10 h-10 rounded-xl flex items-center justify-center flex-shrink-0 bg-teal-500/10">
